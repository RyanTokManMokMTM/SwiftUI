...
struct PostImageCell: View {
    let images:[String]
    let width:CGFloat
    private let widthSpacing  = 6
    
    var body: some View {
        Group{

            if(images.count == 1){
                setimages(filename: images[0])
                .resizable()
                .scaledToFill()
...
struct PostImageCellRow: View { // used for show mulitiple images
    let images:[String]
    let width:CGFloat
...
struct PostImageCell_Previews: PreviewProvider {
    static var previews: some View {
        let image:[String] = user.list[0].PostImages
        let width:CGFloat  = UIScreen.main.bounds.width
        return  Group{
            PostImageCell(images: Array(image[0...1]), width: width)
        }
